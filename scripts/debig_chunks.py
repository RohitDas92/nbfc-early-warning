...
tokens = [count_tokens(p.text) for p in pieces]
print(f"tokens  min/median/max : {min(tokens)} / {int(statistics.median(tokens))} / {max(tokens)}")
print(f"over {MAX_TOKENS}: {sum(1 for t in tokens if t > MAX_TOKENS)}"
      f"   under {MIN_TOKENS}: {sum(1 for t in tokens if t < MIN_TOKENS)}")

unnumbered = [p for p in pieces if not p.number]
...
terms = sorted({m.group(0) for p in pieces for m in candidate.finditer(p.text)})
print("  " + ", ".join(terms[:40]))
